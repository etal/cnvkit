...
def do_heatmap(
    ...
    if r_start is not None or r_end is not None:
        logging.info(
            "Showing log2 ratios in range %s:%s-%s", r_chrom, r_start or 0, r_end or "*"
        )
    elif r_chrom:
    ...
    for i, cnarr in enumerate(cnarrs):
        ...
        if r_chrom:
            ...
            chrom_sizes[r_chrom] = max(
                subcna.end.iat[-1] if subcna else 0, chrom_sizes.get(r_chrom, 0)
            )
        else:
            for chrom, subcna in cnarr.by_chromosome():
                sample_data[i][chrom] = cna2df(subcna)
                chrom_sizes[chrom] = max(
                    subcna.end.iat[-1] if subcna else 0, chrom_sizes.get(r_chrom, 0)
                )
    ...
    if show_range:
        ...
        for i, sample in enumerate(sample_data):
            ...
            if not len(sampl_crow):
                logging.warning(
                    "Sample #%d has no data points in selection %s", i + 1, show_range
                )
            sampl_crow["start"] *= MB
            sampl_crow["end"] *= MB
        ...
        chrom_offsets = plots.plot_chromosome_dividers(
            axis, chrom_sizes, 1, along="y" if vertical else "x"
        )
        # Plot the individual probe/segment coverages
        for i, sample in enumerate(sample_data):
            all_crows = []
    ...
    # Without this, further log2_df.end.iat[-1] causes an IndexError.
    if not len(log2_df):
        return axis
