...
def correlate_cnv_expression(cnv_fname, expression_fname):
    ...
    result = (
        pd.DataFrame(
            ...
            index=cnv_table.index,
        )
        .clip(lower=0)
        .fillna(0)
    )
    result.insert(0, "hugo_gene", cnv_table["Hugo_Symbol"].fillna("").to_numpy())
    return result
...
def load_tcga_table(fname, shared_key):
    ...
    before_pipe = before("|")
    sort_order = (
        table["Hugo_Symbol"].apply(lambda x: (len(x), before_pipe(x))).argsort()
    )
    table = (
        table.iloc[sort_order]
