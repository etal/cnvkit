...
"""Update CNVkit .cnn files to include 'depth' and 'ratio' columns.
...
for fname in args.cnn_files:
    ...
    # Center ratios (already converted from log2), but leave log2 the same
    orig_log2 = cnarr['log2'].copy()
    cnarr.center_all(skip_low='antitargetcoverage' not in fname)
    cnarr['log2'] = orig_log2
    cnarr.sort_columns()
    # DBG
    # print(fname, "descriptives:")
    # print(cnarr.data.describe(include=[np.float_]))
